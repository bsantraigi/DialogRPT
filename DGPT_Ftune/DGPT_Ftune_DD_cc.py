...
print(f'Finetuning DGPT on DD_cc with LR:{LR} and EPOCHS:{EPOCHS}')
...
def finetune(model,lr,epochs,reset_patience=5):
  ...
  for e in range(epochs):
    ...
    #         'val_loss': val_loss,
    #         }, f'DGPT_ftune_dd_cc_chkpt_{e}.pth.tar')

    if val_loss<best_loss:
      patience = reset_patience
      best_loss = val_loss
      torch.save(model.state_dict(), f'DGPT_ftune_dd_cc_chkpt_best_{e}.pkl')
    else:
      patience-=1
    if patience<0:
...
train_dataset = Custom_Dataset('preprocessed_data/DD_cc/dd_cc_train_input.tsv')
valid_dataset = Custom_Dataset('preprocessed_data/DD_cc/dd_cc_validation_input.tsv')
